...
cmd = "cat {} | head -1000 >temp.vcf".format(vcf)
subprocess.call(cmd, shell=True)
f_log.write(cmd + "\n")
...
for pop in pop_sample_dict.keys():
    f_out = open("{}_format.sh".format(pop), "w")
    f_out.write("#!/bin/bash\n")
    f_out.write("#SBATCH --job-name={}_format\n".format(pop))
    ...
    f_out.write("#SBATCH --output={}_format.out\n".format(pop))
    f_out.write("#SBATCH --error={}_format.err\n".format(pop))
    f_out.write("#SBATCH --partition=cluster\n")

    f_out.write("cd /gxfs_work1/fs1/work-geomar/smomw353/popGenomics/A_WGR_final/19_pi/00_analysis\n")


    cmd = "./format.py /gxfs_work1/fs1/work-geomar/smomw353/popGenomics/A_WGR_final/19_pi/{0}_sitepi.sites.pi ./{0}_out.txt {0}".format(pop)

    f_out.write("{}\n".format(cmd))
    f_out.close()


    cmd = "sbatch {}_format.sh".format(pop)
    #subprocess.call(cmd, shell=True)
    f_log.write(cmd + "\n")
